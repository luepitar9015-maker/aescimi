...
echo Recuerda que la contrasena es: Automatizador2026*
echo (Al escribirla no se veran asteriscos, solo escribela y dale a Enter)
echo.

ssh -t cimi@192.168.8.164 "echo '1. Limpiando servicios del sistema viejo (por si quedaron de root)...' && sudo pm2 kill 2>/dev/null ; pm2 kill 2>/dev/null && echo '2. Ingresando al repositorio nuevo...' && cd ~/aescimi && echo '3. Ejecutando actualización y levantando todo...' && chmod +x ACTUALIZAR_DESDE_GITHUB_LINUX.sh && ./ACTUALIZAR_DESDE_GITHUB_LINUX.sh"
