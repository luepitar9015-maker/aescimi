...
```mermaid
graph TD
    A[Visor Cliente: React + Canvas] -->|Envío de PDF/Recortes| B[Backend Server: Express + Node.js]
    B -->|Consulta Tipología y TRD| C[Base de Datos: PostgreSQL]
    B -->|API Request con Contexto| D[Google Gemini 1.5 Flash]
    D -->|Metadatos Estructurados & Confianza| B
    B -->|Orquestación RPA: Puppeteer| E[Portal ECM Institucional]
    B -->|Sincronización Directa| F[Almacenamiento OneDrive]
```
